      ...
      >
        ...
        <div className="px-5 pt-2 pb-4 border-b border-tg-border/50 flex-shrink-0 bg-tg-bg/95 backdrop-blur-md z-10">
          <div className="flex items-start justify-between">
            <div>
              <h3 className="text-[18px] font-bold text-tg-text ">{t('event_detail')}</h3>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          {event.invalid_signature && (
            <div className="p-3.5 rounded-[16px] bg-amber-500/10 border border-amber-500/20 flex items-start gap-3">
              ...
              <div>
                <p className="text-[13px] font-bold text-amber-500">{t('invalid_signature')}</p>
                <p className="text-[12px] text-amber-500/80 mt-0.5 leading-snug">
                  {t('signature_not_verified')}
                </p>
              </div>
            </div>
          )}
          ...
          <Section title={t('general_info')}>
            <Row label={t('type')} value={event.eventType} mono />
            <Row 
              label={t('date')} 
            ...
            />
            <Row label={t('summary')} value={event.summary || '—'} />
            <Row label={t('processed')} value={event.processed ? t('yes') : t('no')} highlight={!event.processed} />
          </Section>
          ...
          {isSubscription && event.resource && (
            <Section title={t('subscription')}>
              ...
              <Row label={t('status')} value={event.resource.status || '—'} highlight />
              <Row label={t('plan_id')} value={event.resource.plan_id || '—'} mono />
              {event.resource.start_time && (
                <Row label={t('start')} value={new Date(event.resource.start_time).toLocaleDateString(undefined)} />
              )}
              {event.resource.status_update_time && (
                <Row label={t('last_change')} value={new Date(event.resource.status_update_time).toLocaleString(undefined)} />
              )}
            </Section>
          )}
          ...
          {isPayment && event.resource && (
            <Section title={t('payment_details')}>
              <Row label="Transaction ID" value={event.resource.id || '—'} mono />
              <Row label={t('status')} value={event.resource.state || '—'} highlight />
              ...
              {event.resource.payment_mode && (
                <Row label={t('mode')} value={event.resource.payment_mode} />
              )}
              {event.resource.billing_agreement_id && (
                <Row label={t('subscription')} value={event.resource.billing_agreement_id} mono />
              )}
            </Section>
          )}
          ...
          {/* Pagador */}
          {event.subscriber && (
            <Section title={t('payer')}>
              {event.subscriber.name && (
                <Row label={t('name')} value={`${event.subscriber.name.given_name} ${event.subscriber.name.surname}`} />
              )}
          ...
          {/* IDs Internos */}
          <Section title={t('internal_ids')}>
            <Row label="Event ID" value={event.event_id} mono small />
            <Row label={t('subscription_ref')} value={event.subscriptionId} mono />
          </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2.5">
      <h4 className="text-[12px] font-bold text-tg-hint uppercase  pl-2">{title}</h4>
