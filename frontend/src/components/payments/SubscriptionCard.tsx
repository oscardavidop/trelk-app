...
const STATUS_STYLES: Record<string, { bg: string; text: string; border: string; label: string }> = {
  ACTIVE: { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/20', label: 'Activa' },
  CANCELLED: { bg: 'bg-red-500/10', text: 'text-red-400', border: 'border-red-500/20', label: 'Cancelada' },
  CANCEL_PENDING: { bg: 'bg-amber-500/10', text: 'text-amber-400', border: 'border-amber-500/20', label: 'Cancelando...' },
  SUSPENDED: { bg: 'bg-orange-500/10', text: 'text-orange-400', border: 'border-orange-500/20', label: 'Suspendida' },
  EXPIRED: { bg: 'bg-zinc-500/10', text: 'text-zinc-400', border: 'border-zinc-500/20', label: 'Expirada' },
  APPROVAL_PENDING: { bg: 'bg-blue-500/10', text: 'text-blue-400', border: 'border-blue-500/20', label: 'Pendiente' },
};
...
export default function SubscriptionCard({
    ...
    return (
      <div className="rounded-[24px] bg-tg-secondary border border-tg-border/30 p-6 shadow-lg">
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            <p className="text-[16px] font-bold text-tg-text tracking-tight">Sin suscripción activa</p>
            <p className="text-[13px] text-tg-hint/80 mt-0.5">No tienes un plan premium actualmente</p>
          </div>
        </div>
  ...
  const status = STATUS_STYLES[subscription.status] || STATUS_STYLES.ACTIVE;
  const nextBilling = subscription.next_billing_date
    ? new Date(subscription.next_billing_date).toLocaleDateString('es', { day: '2-digit', month: 'short', year: 'numeric' })
    : null;
  const startDate = subscription.start_time
    ? new Date(subscription.start_time).toLocaleDateString('es', { day: '2-digit', month: 'short', year: 'numeric' })
    : null;
    ...
    <div className="rounded-[24px] bg-tg-secondary border border-tg-border/30 overflow-hidden shadow-xl">
      ...
      <div className="bg-gradient-to-br from-tg-accent/15 via-tg-secondary to-transparent p-5 border-b border-white/5">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <p className="text-[16px] font-extrabold text-tg-text tracking-tight">Suscripción Premium</p>
              <p className="text-[13px] font-medium text-tg-hint/80 mt-0.5">Plan mensual · PayPal</p>
            </div>
          </div>
          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border ${status.bg} ${status.text} ${status.border}`}>
            {status.label}
          </span>
        </div>
      </div>
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-y-4 gap-x-2">
          <InfoRow label="Precio" value={`$${subscription.amount} ${subscription.currency}`} />
          <InfoRow label="Inicio del Plan" value={startDate || '—'} />
          <InfoRow label="Frecuencia" value="Mensual" />
          <InfoRow label="Próximo Cobro" value={nextBilling || '—'} highlight />
        </div>
          ...
          <div className="flex gap-2.5 pt-4 border-t border-white/5">
            {totalSpent.map((s) => (
              <div key={s.currency} className="flex-1 bg-white/[0.03] rounded-[16px] p-3 text-center border border-white/[0.02]">
                <p className="text-[17px] font-extrabold text-tg-text tabular-nums tracking-tight">${s.total.toFixed(2)}</p>
            ...
            <div className="flex-1 bg-white/[0.03] rounded-[16px] p-3 text-center border border-white/[0.02]">
              <p className="text-[17px] font-extrabold text-tg-text tabular-nums tracking-tight">{totalSubscriptions}</p>
              <p className="text-[10px] font-medium text-tg-hint mt-0.5 uppercase tracking-wider">Suscripciones</p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3 pt-2">
          ...
          {subscription.status === 'ACTIVE' && onCancel && (
            ...
            >
              <XCircle size={16} />
              {cancelling ? 'Procesando...' : 'Cancelar Plan'}
            </button>
          )}
        </div>
      </div>
...
function InfoRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex flex-col gap-1">
      <span className="text-[10px] font-bold text-tg-hint/70 uppercase tracking-widest">{label}</span>
      <span className={`text-[15px] tabular-nums tracking-tight ${highlight ? 'text-tg-accent font-extrabold' : 'text-tg-text font-semibold'}`}>
        {value}
      </span>
    </div>
  );
