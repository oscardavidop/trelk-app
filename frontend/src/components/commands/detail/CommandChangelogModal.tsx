...
import { cmdSlug, findCommand } from '@/data/botCommands';
import { VersionCard } from './CommandChangelogEnhanced';
import { loadChangelog, hasChangelog } from '../../../utils/changelogLoader';
import { ParsedChangelog } from '@/utils/parseChangelog';
import { useLocation, useNavigate } from 'react-router-dom';

interface CommandChangelogModalProps {
    isOpen: boolean;
    // onClose: () => void;
    slug: string;
}

export default function CommandChangelogModal({ isOpen, slug }: CommandChangelogModalProps) {
    ...
    //   addEventLister on back button to close modal instead of navigating back
    const location = useLocation();
    const navigate = useNavigate();

    // El modal está abierto SI el hash actual coincide
    const isOpened = location.hash === "#changelog";
...
    }, [isOpened]);
    ...
    useEffect(() => {
        // Solo cargamos si el modal está abierto y hay slug
        if (!isOpen) return;

        if (!mainSlug || !hasChangelog(mainSlug)) {
            setLoading(false);
            return;
        }

        setLoading(true);
        loadChangelog(mainSlug).then(d => {
            setData(d);
            setLoading(false);
        });
    }, [mainSlug, isOpen]);

    // Si no está abierto, no montamos el contenido pesado (AnimatePresence maneja la salida)
    ...
    if (!isOpened) return null; // No renderizamos nada si no está abierto

    return (
        <AnimatePresence>
            {isOpened && (
                <motion.div
                    initial={{ opacity: 0, x: 20 }} // Animación de entrada estilo navegación
                ...
                >
                    ...
                    >
                        ...
                        <div className="px-5 py-3.5 flex items-center gap-3">
                            <button
                                type="button"
                                onClick={handleClose} // <-- Aquí usamos handleClose en vez de navigate(-1)
