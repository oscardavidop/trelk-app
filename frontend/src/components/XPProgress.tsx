...
export default function XPProgress({ compact, rounded }: XPProgressProps) {
    ...
    return (
      <div className="flex items-center gap-3.5 group cursor-default">
        ...
        <div className="relative shrink-0">
          ...
          {streak > 1 && (
            <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-[#1c1c1e] border border-white/10 flex items-center justify-center shadow-md z-10">
        ...
        <div className="flex-1 min-w-0 pt-0.5">
          ...
          {/* Fondo de barra más profundo para mejor contraste */}
          <div className="h-1.5 rounded-full bg-black/30 border border-white/5 overflow-hidden shadow-inner">
            <div
            ...
            >
              {/* Brillo superior fino (tubo de luz), igual que los otros modos */}
  ...
  if (rounded) {
    return (
      <div className="w-auto max-w-[130px] h-[58px] rounded-full bg-gradient-to-b from-tg-secondary to-tg-secondary/80 border border-white/10 flex items-center p-1 shadow-md relative overflow-hidden backdrop-blur-md cursor-default">
        ...
        <div className="w-[42px] h-[42px] rounded-full bg-gradient-to-br from-amber-400 to-orange-500 p-[1.5px] shrink-0 relative z-10 shadow-[0_2px_8px_rgba(245,158,11,0.5)]">
          {/* Textura interna de la esfera */}
          <div className="w-full h-full rounded-full border border-white/30 bg-gradient-to-b from-white/10 to-transparent flex items-center justify-center shadow-inner">
            <span className="text-[16px] font-black text-white drop-shadow-md leading-none tracking-tight mt-0.5">
              {level}
            </span>
          </div>
          ...
          {streak > 1 && (
            <div className="absolute -bottom-0.5 -right-0.5 w-[18px] h-[18px] rounded-full bg-[#1c1c1e] border border-white/10 flex items-center justify-center shadow-sm z-20">
        ...
        {/* ── Derecha: Textos y Barra ── */}
        <div className="flex-1 flex flex-col justify-center px-2 z-10 min-w-0">

          {/* Fila superior: XP a la izquierda, Porcentaje a la derecha */}
          <div className="flex items-end justify-between w-full mb-1.5 px-0.5">
            <span className="text-[11px] font-extrabold text-tg-text drop-shadow-sm leading-none tracking-tight">
              {currentXP}/{nextXP} <span className="text-[8px] text-tg-hint font-bold uppercase">XP</span>
            </span>
            <span className="text-[10px] font-black text-amber-500 drop-shadow-sm leading-none">
              {Math.round(progress * 100)}%
            </span>
          </div>

          {/* Mini barra tipo neón */}
          <div className="w-full h-1.5 rounded-full bg-black/40 shadow-inner overflow-hidden">
            <div
