var Main = {
  init(api_hash = null) {
    if (api_hash) {
      Aj.apiUrl = '/users/api?hash=' + api_hash;
    }
    Main.initOnce();
...
var CreateBot = {
  ...
  submit() {
    ...
    Aj.apiRequest('createBot', {
      ...
      if (res.ok) {
        Aj.location(`/users/ui/${res.bot_id}`);
...
var BotProfile = {
  ...
  eMainClick() {
    BotProfile.submit(`/users/ui/${Aj.state.botId}`);
  },
  ...
  init() {
    ...
    $('.js-edit-command-list').on('click', '.tm-row-link', function () {
      ...
      if (!Aj.state.edit && command) {
        ...
        Aj.location('/users/ui/' + Aj.state.botId + '/commands/' + command + q);
      }
    });
...
var BotSettings = {
  init() {
    ...
    function updateAdminRights() {
      ...
      botChangeSettings('notifications_settings', new_values);
    }
    ...
    function updateBroadcastAdminRights() {
      ...
      botChangeSettings('chat_actions', new_values);
    }
...
var BotGameEdit = {
  ...
  submit() {
    ...
    Aj.apiRequest('setApp', {
      ...
      if (res.ok) {
        Aj.onUnload(() => Main.showSuccessToast(res.msg));
        Aj.location(`/users/ui/${Aj.state.botId}/games`);
      } else if (res.error) {
        Main.showErrorToast(res.error);
      }
    });
...
var BotSettingsPreferences = {
  init() {
    $('[name=auto_detect_lang]').on('click', function () {
      const isOn = $(this).hasClass('tm-toggle-on');
      $('[name=default_lang]').toggle(isOn); // si estaba encendido, ahora se apaga
    });
    ...
    $('input[name="lang_option"]').on('change', function (e) {
      $('input[name="lang_option"]').prop('checked', false);
      $(this).prop('checked', true);
      BotSettingsPreferences.eClickLang(e);
    });

    $('input[name="tz_option"]').on('change', function (e) {
      $('input[name="tz_option"]').prop('checked', false);
      $(this).prop('checked', true);
      BotSettingsPreferences.eClickTz(e);
    });

    let currentLang = $('[name="default_lang"]').data('value');
    $(`input[name="lang_option"][value="${currentLang}"]`).prop('checked', true);
    $('[name="current_lang_status"]').text(langs[currentLang].name)

    let currentTz = $('[name="default_tz"]').attr('data-value');
    // $('[name="current_tz_status"]').text(`${timezones[currentTz].label} (${timezones[currentTz].description})`);
    $(`input[name="tz_option"][value="${currentTz}"]`).prop('checked', true);
  },
  eUploadClick() {
    var target = this.dataset.target;
    if (!target) return;
  ...
  eClickTz(e) {
    var tz = $(e.target).val();
    var ct = $('[name="current_tz"]').attr('data-value');
    console.log(tz, ct);
    if (tz == ct) return;
    $('[name="current_tz"]').attr('data-value', tz);
    var href = '?tz=' + tz;
    BotSettingsPreferences.submit(false, { tz });
  },
  eClickLang(e) {
    var lang = $(e.target).val();
    if (lang == Aj.state.lang) return;
    $('[name="current_lang"]').attr('data-value', lang);
    BotSettingsPreferences.submit('./lang', { lang });
  },
  hasChanges() {
    var old = Aj.state.savedModel;
    var model = BotProfile.model();
  ...
  submit(redirect = false, data = {}) {
    WebApp.MainButton.showProgress();
    var model = BotSettingsPreferences.model();
    Aj.apiRequest('updateConfig', {
      config: {
        ...model,
        ...data
      }
...
var BotAppEdit = {
  ...
  submit() {
    ...
    Aj.apiRequest('setApp', {
      ...
      if (res.ok) {
        Aj.onUnload(() => Main.showSuccessToast(res.msg));
        Aj.location(`/users/ui/${Aj.state.botId}/apps`);
      } else if (res.error) {
        Main.showErrorToast(res.error);
      }
    });
...
(function () {
  ...
  const LOG_MARK = '__from_iframe__'; // marca única

  console.log = function (...args) {
    // Agregamos la marca para que el padre sepa que viene del iframe
    window.parent.postMessage({ type: 'iframe-log', args, mark: LOG_MARK }, '*');
    // Solo mostramos en la consola del iframe si no está embebido
...
var langs = {
  ...
  "it": { "name": "Italiano", "englishName": "Italian" },
  "pt": { "name": "Português", "englishName": "Portuguese" },
  "ru": { "name": "Русский", "englishName": "Russian" },
  "zh-CN": { "name": "简体中文", "englishName": "Chinese (Simplified)" },
  "zh-TW": { "name": "繁體中文", "englishName": "Chinese (Traditional)" },
  "ja": { "name": "日本語", "englishName": "Japanese" },
  "ko": { "name": "한국어", "englishName": "Korean" },
  "ar": { "name": "العربية", "englishName": "Arabic" },
